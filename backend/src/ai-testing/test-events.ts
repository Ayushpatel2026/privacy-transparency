...
import { DataDestination, DataSource, DataType, EncryptionMethod, TransparencyEvent, UserConsentPreferences } from "../constants/types/Transparency";
...
// Helper function to get test case by key
export function getTestCase(key: string): TestCase | undefined {
  return testEvents.get(key);
}

// Helper function to get all test case keys
export function getTestCaseKeys(): string[] {
  return Array.from(testEvents.keys());
}