...
router.post('/', verifyToken as RequestHandler, async (req: Request, res: Response) => {
    try {
        console.log("Creating sleep data in the backend with data:", req.body);
        if (!req.userId) {
        ...
        if (existingSleepData) {
            ...
            if (!sleepData) {
                res.status(404).json({ message: 'General sleep data not found or failed to update.' });
                return;
            }
...
router.delete('/', verifyToken as RequestHandler, async (req: Request, res: Response) => {
    try {
        ...
        res.status(204).json({ message: 'General sleep data deleted successfully.' });
